...
if(isset($_POST['add_comment'])){
   ...
   $admin_id = filter_var($admin_id, ENT_QUOTES, 'UTF-8');
   $user_name = $_POST['user_name'];
   $user_name = filter_var($user_name, ENT_QUOTES, 'UTF-8');
   $comment = $_POST['comment'];
   $comment = filter_var($comment, ENT_QUOTES, 'UTF-8');
...
<head>
   ...
   <title>Посмотреть публикацию</title>
...
?>
   ...
   <form action="" method="POST">
      ...
      <textarea name="comment_edit_box" required cols="30" rows="10" placeholder="Введите ваш комментарий"><?= $fetch_edit_comment['comment']; ?></textarea>
      <button type="submit" class="inline-btn" name="edit_comment">Изменить</button>
      <div class="inline-option-btn" onclick="window.location.href = 'view_post.php?post_id=<?= $get_id; ?>';">Отмена</div>
   </form>
   </section>
<?php
   }
...
<section class="posts-container" style="padding-bottom: 0;">
   ...
   <div class="box-container">
      ...
      <?php
         ...
         echo '<p class="empty">Публикаций не найдено!</p>';
      }
      ?>
   </div>
...
<section class="comments-container">
   ...
   <p class="comment-title">Написать комментарий</p>
   <?php
      if($user_id != ''){  
   ...
   <form action="" method="post" class="add-comment">
      ...
      <textarea name="comment" maxlength="1000" class="comment-box" cols="30" rows="10" placeholder="Напишите комментарий" required></textarea>
      <input type="submit" value="Опубликовать" class="inline-btn" name="add_comment">
   </form>
   <?php
   }else{
   ?>
   <div class="add-comment">
      <p>Войдите чтобы написать или изменить комментарий</p>
      <a href="login.php" class="inline-btn">Войти</a>
   </div>
   <?php
      }
   ?>
   
   <p class="comment-title">Комментарии к публикации</p>
   <div class="user-comments-container">
      ...
      <div class="show-comments" style="<?php if($fetch_comments['user_id'] == $user_id){echo 'order:-1;'; } ?>">
         ...
         <form action="" method="POST">
            ...
            <button type="submit" class="inline-delete-btn" name="delete_comment" onclick="return confirm('Удалить комментарий?');">Удалить</button>
         </form>
         <?php
         }
         ?>

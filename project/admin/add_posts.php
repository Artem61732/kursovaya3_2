<?php

include '../components/connect.php';

session_start();

$admin_id = $_SESSION['admin_id'];

if(!isset($admin_id)){
   header('location:admin_login.php');
}

if(isset($_POST['publish'])){

   $name = $_POST['name'];
   $name = filter_var($name, ENT_QUOTES, 'UTF-8');
   $title = $_POST['title'];
   $title = filter_var($title, ENT_QUOTES, 'UTF-8');
   $content = $_POST['content'];
   $content = filter_var($content, ENT_QUOTES, 'UTF-8');
   $category = $_POST['category'];
   $category = filter_var($category, ENT_QUOTES, 'UTF-8');
   $status = 'active';
   
   $image = $_FILES['image']['name'];
   $image = filter_var($image, ENT_QUOTES, 'UTF-8');
   $image_size = $_FILES['image']['size'];
   $image_tmp_name = $_FILES['image']['tmp_name'];
   $image_folder = '../uploaded_img/'.$image;

   $select_image = $conn->prepare("SELECT * FROM `posts` WHERE image = ? AND admin_id = ?");
   $select_image->execute([$image, $admin_id]);

   if(isset($image)){
      if($select_image->rowCount() > 0 AND $image != ''){
         $message[] = 'Название изображения повторяется!';
      }elseif($image_size > 2000000){
         $message[] = 'Размер изображения слишком большой!';
      }else{
         move_uploaded_file($image_tmp_name, $image_folder);
      }
   }else{
      $image = '';
   }

   if($select_image->rowCount() > 0 AND $image != ''){
      $message[] = 'Переименуйте свое изображение!';
   }else{
      $insert_post = $conn->prepare("INSERT INTO `posts`(admin_id, name, title, content, category, image, status) VALUES(?,?,?,?,?,?,?)");
      $insert_post->execute([$admin_id, $name, $title, $content, $category, $image, $status]);
      $message[] = 'Пост опубликован!';
   }
   
}

if(isset($_POST['draft'])){

   $name = $_POST['name'];
   $name = filter_var($name, ENT_QUOTES, 'UTF-8');
   $title = $_POST['title'];
   $title = filter_var($title, ENT_QUOTES, 'UTF-8');
   $content = $_POST['content'];
   $content = filter_var($content, ENT_QUOTES, 'UTF-8');
   $category = $_POST['category'];
   $category = filter_var($category, ENT_QUOTES, 'UTF-8');
   $status = 'deactive';
   
   $image = $_FILES['image']['name'];
   $image = filter_var($image, ENT_QUOTES, 'UTF-8');
   $image_size = $_FILES['image']['size'];
   $image_tmp_name = $_FILES['image']['tmp_name'];
   $image_folder = '../uploaded_img/'.$image;

   $select_image = $conn->prepare("SELECT * FROM `posts` WHERE image = ? AND admin_id = ?");
   $select_image->execute([$image, $admin_id]); 

   if(isset($image)){
      if($select_image->rowCount() > 0 AND $image != ''){
         $message[] = 'Название изображения повторяется!';
      }elseif($image_size > 2000000){
         $message[] = 'Размер изображения слишком большой!';
      }else{
         move_uploaded_file($image_tmp_name, $image_folder);
      }
   }else{
      $image = '';
   }

   if($select_image->rowCount() > 0 AND $image != ''){
      $message[] = 'Переименуйте свое изображение!';
   }else{
      $insert_post = $conn->prepare("INSERT INTO `posts`(admin_id, name, title, content, category, image, status) VALUES(?,?,?,?,?,?,?)");
      $insert_post->execute([$admin_id, $name, $title, $content, $category, $image, $status]);
      $message[] = 'Сохранено!';
   }

}

?>

<!DOCTYPE html>
<html lang="en">
<head>
   <meta charset="UTF-8">
   <meta http-equiv="X-UA-Compatible" content="IE=edge">
   <meta name="viewport" content="width=device-width, initial-scale=1.0">
   <title>Добавить публикацию</title>

   <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.1.1/css/all.min.css">

   <link rel="stylesheet" href="../css/admin_style.css">

</head>
<body>



<?php include '../components/admin_header.php' ?>

<section class="post-editor">

   <h1 class="heading">Добавить новую публикацию</h1>

   <form action="" method="post" enctype="multipart/form-data">
      <input type="hidden" name="name" value="<?= $fetch_profile['name']; ?>">
      <p>Название публикации <span>*</span></p>
      <input type="text" name="title" maxlength="100" required placeholder="Добавить название публикации" class="box">
      <p>Наполнение публикации <span>*</span></p>
      <textarea name="content" class="box" required maxlength="10000" placeholder="Напишите ваш текст..." cols="30" rows="10"></textarea>
      <p>Категория публикации <span>*</span></p>
      <select name="category" class="box" required>
         <option value="" selected disabled>-- Выбрать категорию* </option>
         <option value="painting">Живопись</option>
         <option value="glass">Стекло</option>
         <option value="sculpture">Скульптура</option>
      </select>
      <p>Изображение публикации</p>
      <input type="file" name="image" class="box" accept="image/jpg, image/jpeg, image/png, image/webp">
      <div class="flex-btn">
         <input type="submit" value="Опубликовать" name="publish" class="btn">
         <input type="submit" value="Сохранить черновик" name="draft" class="option-btn">
      </div>
   </form>

</section>

<script src="../js/admin_script.js"></script>

</body>
</html>
...
if(isset($_POST['delete'])){
   ...
   $p_id = $_POST['post_id'];
   $p_id = filter_var($p_id, ENT_QUOTES, 'UTF-8');
...
<section class="read-post">
   ...
   <form method="post">
      ...
      <div class="flex-btn">
         <a href="edit_post.php?id=<?= $post_id; ?>" class="inline-option-btn">Изменить</a>
         <button type="submit" name="delete" class="inline-delete-btn" onclick="return confirm('Удалить эту публикацию?');">Удалить</button>
         <a href="view_posts.php" class="inline-option-btn">Назад</a>
      </div>
   </form>
   <?php
         }
      }else{
         echo '<p class="empty">Публикации ещё не добавлены! <a href="add_posts.php" class="btn" style="margin-top:1.5rem;">Добавить</a></p>';
      }
   ?>
...
<section class="comments" style="padding-top: 0;">
   ...
   <div class="box">
      ...
      <form action="" method="POST">
         ...
         <button type="submit" class="inline-delete-btn" name="delete_comment" onclick="return confirm('Удалить этот комментарий?');">Удалить</button>
      </form>
   </div>
   <?php
         }
      }else{
         echo '<p class="empty">Комментарии ещё не добавлены!</p>';
      }
   ?>
   </div>
